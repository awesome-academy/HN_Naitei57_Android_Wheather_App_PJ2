...
class HomeViewModel(
    ...
    fun fetchCurrentWeather(latitude: Double, longitude: Double,language:String) {
    ...
    fun fetchWeatherForSearchResult(location: String,language:String){
        isLoading.value = true
        viewModelScope.launch {
            try {
                val weatherDeferred =
    ...
    fun addFavouriteLocation() {
        ...
        currentWeather.value?.let { weather ->
            ...
            viewModelScope.launch {
                val cnt = weatherRepository.isFavoriteLocationExists(cityName!!)
                if (cnt > 0) {
